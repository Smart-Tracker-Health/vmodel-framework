...
- [ ] **NFA-Bibliothekszitate verifizieren (P-31):** Für jede NFA, die eine konkrete Implementierungs-Bibliothek nennt, prüfen ob sie tatsächlich verwendet wird — per Source-Grep gegen `import`-/Include-Statements und per Suche im Dependency-Manifest. Wenn die NFA-Lib weder als Import noch als Dependency erscheint → Editorial CR aufmachen. Stack-spezifische Greps siehe `patterns.md`.
